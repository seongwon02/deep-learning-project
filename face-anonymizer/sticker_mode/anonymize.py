...
def run_face_crop_inpaint(
    ...
    for face_index, detection in enumerate(targets):
        ...
        if getattr(args, "sticker_anonymize", False) and track_id is not None:
            if track_id in getattr(args, "_sticker_cache", {}):
                use_sticker = True
...
def process_video(args: argparse.Namespace, detections_by_frame: dict[int, list[FaceDetection]]) -> None:
    ...
    # Sticker Cache & Lazy Loading Pipeline Initializations
    args._sticker_cache = {}
    args._pipe = None
    args._device = "cpu"

    if not args.mask_preview and not args.sticker_anonymize:
        print("Loading baseline diffusion pipeline immediately...")
    ...
    try:
        while True:
            ...
            if args.mask_preview:
                ...
                if args.inpaint_scope == "face-crop":
                    output_image = run_face_crop_inpaint(args._pipe, args._device, image, detections, frame_index, args)
                else:
                    identity = choose_identity(None, args.identity_bank_entries, 0)
                    seed = seed_for_context(
                        base_seed=args.seed,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sticker-anonymize", action="store_true", help="Enable fast sticker-based anonymization.")
